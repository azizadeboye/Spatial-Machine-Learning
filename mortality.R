...
qtm(nat, text = "NAME")  # Replace with actual field name
...
library(readxl)
mdata <- read_excel("pancre_mort.xlsx")
...
qtm(mapdata, fill = "NAME", fill.legend = tm_legend(title = "Map of Africa")) + 
  tm_compass()
...
# quantile map
w1 <- qtm(mapdata, fill = "mortality")
w2 <- qtm(mapdata, fill = "female")
w3 <- qtm(mapdata, fill = "male")
w4 <- qtm(mapdata, fill = "ageb")
w5 <- qtm(mapdata, fill = "agec")
w6 <- qtm(mapdata, fill = "agea")
w7 <- qtm(mapdata, fill = "fageb")
w8 <- qtm(mapdata, fill = "fagec")
w9 <- qtm(mapdata, fill = "fagea")
w10 <- qtm(mapdata, fill = "mageb")
w11 <- qtm(mapdata, fill = "magec")
w12 <- qtm(mapdata, fill = "magea")
w13 <- qtm(mapdata, fill = "yra")
w14 <- qtm(mapdata, fill = "yrb")
w15 <- qtm(mapdata, fill = "yrc")
w16 <- qtm(mapdata, fill = "yrd")
w17 <- qtm(mapdata, fill = "yre")

current.mode <- tmap_mode("plot")
tmap_arrange(w1, widths = c(.75, .75))
tmap_arrange(w2, w3, widths = c(.75, .75))
tmap_arrange(w4, w5, w6, widths = c(.75, .75))
tmap_arrange(w7, w8, w9, w10, w11, w12, widths = c(.75, .75))
tmap_arrange(w13, w14, w15, w16, w17, widths = c(.75, .75))
tmap_mode(current.mode)

p1 <- tm_shape(mapdata) + 
  tm_fill("mortality", fill.scale =tm_scale_intervals(values = "brewer.reds", style = "quantile"), 
          fill.legend = tm_legend(title = "mortality")) + tm_borders(fill_alpha = .3) + tm_compass() + 
...
# Train Random Forest
set.seed(123)
rf_model <- randomForest(mortality ~ female + male + agea + ageb + agec + fagea + fageb + fagec +
...
library(xgboost)
# Prepare the data
x_vars <- model.matrix(mortality ~ female + male + agea + ageb + agec + fagea + fageb + fagec +
...
# Train SVR model
svr_model <- svm(mortality ~ female + male + agea + ageb + agec + fagea + fageb + fagec +
...
rf_metrics <- postResample(pred = rf_preds, obs = mapdata$mortality)
print(rf_metrics)

# 2. XGBoost Evaluation
xgb_preds <- predict(xgb_model, newdata = x_vars)
xgb_metrics <- postResample(pred = xgb_preds, obs = mapdata$mortality)
print(xgb_metrics)

# 3. SVR Evaluation
svr_preds <- predict(svr_model, newdata = mapdata)
svr_metrics <- postResample(pred = svr_preds, obs = mapdata$mortality)
print(svr_metrics)
...
model_long <- model_preds %>%
  pivot_longer(cols = c("rf", "xgb", "svr"), names_to = "Model", values_to = "Predicted")

# Plot
ggplot(model_long, aes(x = ID, y = Predicted, color = Model)) +
  geom_line(linewidth = 0.5) +
  labs(title = "Model Predictions Over Observations",
       x = "Observation", y = "Predicted Mortality") +
  theme_minimal()
...
# Random Forest
plot(mapdata$mortality, mapdata$rf_pred,
     xlab = "Observed", ylab = "Predicted",
     main = "Random Forest", pch = 19, col = "steelblue")
abline(0, 1, col = "red", lwd = 2)

# XGBoost
plot(mapdata$mortality, mapdata$xgb_pred,
     xlab = "Observed", ylab = "Predicted",
     main = "XGBoost", pch = 19, col = "darkgreen")
abline(0, 1, col = "red", lwd = 2)

# SVR
plot(mapdata$mortality, mapdata$svr_pred,
     xlab = "Observed", ylab = "Predicted",
...
# Random Forest Plot
p1 <- ggplot(mapdata, aes(x = mortality, y = rf_pred)) +
  geom_point(color = "steelblue", alpha = 0.6) +
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +
  stat_cor(method = "pearson", aes(label = paste0("R² = ")), label.x = 0) +
  labs(title = "Random Forest", x = "Observed Mortality", y = "Predicted Mortality") +
  theme_minimal()

# XGBoost Plot
p2 <- ggplot(mapdata, aes(x = mortality, y = xgb_pred)) +
  geom_point(color = "darkgreen", alpha = 0.6) +
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +
  stat_cor(method = "pearson", aes(label = paste0("R² = ")), label.x = 0) +
  labs(title = "XGBoost", x = "Observed Mortality", y = "Predicted Mortality") +
  theme_minimal()

# SVR Plot
p3 <- ggplot(mapdata, aes(x = mortality, y = svr_pred)) +
  geom_point(color = "purple", alpha = 0.6) +
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +
  stat_cor(method = "pearson", aes(label = paste0("R² = ")), label.x = 0) +
  labs(title = "SVR", x = "Observed Mortality", y = "Predicted Mortality") +
  theme_minimal()

combined_plot <- ggarrange(p1, p2, p3, ncol = 3, nrow = 1, common.legend = FALSE)

# Save to PDF
ggsave("Observed_vs_Predicted_All_Models.pdf", plot = combined_plot, width = 12, height = 4)
...
rf_cv <- train(
  mortality ~ female + male + agea + ageb + agec + fagea + fageb + fagec +
...
tm_shape(mapdata) +
  tm_fill("pred_rf", fill.scale =tm_scale_intervals(values = "brewer.yl_gn_bu", style = "quantile"), 
          fill.legend = tm_legend(title = "Mort_pred_rf")) +
  tm_borders() #+
...
tm_shape(mapdata) + 
  tm_fill("pred_rf", fill.scale =tm_scale_intervals(values = "brewer.greens",  style = "quantile"), 
          fill.legend = tm_legend(title = "Mort_pred_rf")) + tm_borders(fill_alpha = .2) + 
...
tm_shape(mapdata) +
  tm_fill("pred_xgb", fill.scale =tm_scale_intervals(values = "brewer.pu_bu", style = "quantile"), 
          fill.legend = tm_legend(title = "Mort_pred_xgb")) +
  tm_borders() #+
...
tm_shape(mapdata) + 
  tm_fill("pred_xgb", fill.scale =tm_scale_intervals(values = "brewer.purples", style = "quantile"), 
          fill.legend = tm_legend(title = "Mort_pred_xgb")) + tm_borders(fill_alpha = .2) + 
  tm_compass() + tm_layout(legend.text.size = 0.5, legend.position = c("left", "bottom"), 
                           frame = TRUE, component.autoscale = FALSE)
...
tm_shape(mapdata) +
  tm_fill("pred_svr", fill.scale =tm_scale_intervals(values = "brewer.gn_bu", style = "quantile"), 
          fill.legend = tm_legend(title = "Mort_pred_svr")) +
  tm_borders() #+
...
tm_shape(mapdata) + 
  tm_fill("pred_svr", fill.scale =tm_scale_intervals(values = "brewer.reds", style = "quantile"), 
          fill.legend = tm_legend(title = "Mort_pred_svr")) + tm_borders(fill_alpha = .2) + 
  tm_compass() + tm_layout(legend.text.size = 0.5, legend.position = c("left", "bottom"), 
                           frame = TRUE, component.autoscale = FALSE)
...
# Random Forest
rf_preds <- predict(rf_model, newdata = mapdata)
rf_resid <- mapdata$mortality - rf_preds

# XGBoost
xgb_preds <- predict(xgb_model, newdata = x_vars)  # x_vars = model.matrix(...)
xgb_resid <- mapdata$mortality - xgb_preds

# SVR
svr_preds <- predict(svr_model, newdata = mapdata)
svr_resid <- mapdata$mortality - svr_preds

### Step 2: Combine into a single data frame
residuals_df <- data.frame(
  actual = mapdata$mortality,
  rf_pred = rf_preds,
  rf_resid = rf_resid,
  xgb_pred = xgb_preds,
  xgb_resid = xgb_resid,
...
map_rf <- tm_shape(mapdata) +
  tm_fill("rf_resid", fill.scale =tm_scale_intervals(values = "brewer.greens", style = "quantile", 
    midpoint = 0), fill.legend = tm_legend(title = "mort_rf_resid")) +
...
map_xgb <- tm_shape(mapdata) + 
  tm_fill("xgb_resid", fill.scale =tm_scale_intervals(values = "brewer.purples", style = "quantile", 
    midpoint = 0), fill.legend = tm_legend(title = "Mort_xgb_resid")) + tm_borders(fill_alpha = .2) + 
...
map_svr <- tm_shape(mapdata) + 
  tm_fill("svr_resid", fill.scale =tm_scale_intervals(values = "brewer.reds", style = "quantile"), 
          fill.legend = tm_legend(title = "Mort_svr_resid")) + tm_borders(fill_alpha = .2) + 
  tm_layout(legend.text.size = 0.5, legend.position = c("left", "bottom"), 
                           frame = TRUE, component.autoscale = FALSE)
...
#Optional: Visualize MAE and RMSE
par(mfrow = c(1, 3))
#Barplot of RMSE
...
#Bonus: Add metrics to residual maps as captions
map_rf <- tm_shape(mapdata) +
